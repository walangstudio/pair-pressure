...
Run:
```
pp resolve --server <server> --channel <ch> --thread <id> [--outcome "<outcome>"]
```
...
- `{"ok": false, "reason": "decision_needs_enum_outcome", "valid": [...]}` — outcome must be one of the listed values.
