...
Use the **current joined thread** (server + channel + thread_id) from this session's context. If none, refuse and ask the user to `/pp-chat:join` or `/pp-chat:new` first — do not guess a thread.

Run:
```
pp reply --server <server> --channel <ch> --thread <id> --stance extend --via human --body-file -
```
